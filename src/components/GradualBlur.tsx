...
export interface GradualBlurProps extends Partial<GradualBlurConfig> {
  mobileHeight?: string
  tabletHeight?: string
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const records = config as unknown as Record<string, string | undefined>
    const cap = key.charAt(0).toUpperCase() + key.slice(1)
    const calc = () => {
      const w = window.innerWidth
      let v = config[key]
      if (w <= 480 && records[`mobile${cap}`]) v = records[`mobile${cap}`]
      else if (w <= 768 && records[`tablet${cap}`]) v = records[`tablet${cap}`]
      else if (w <= 1024 && records[`desktop${cap}`]) v = records[`desktop${cap}`]
      setValue(v)
    }
